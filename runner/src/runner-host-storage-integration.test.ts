 ...
 *     exported openEngineDatabase: PRAGMA user_version === 6 + the migration-1
...
const tableCountsAt = (dbPath: string): Record<string, number> => {
  ...
  try {
    ...
    for (const table of [...MIGRATION_1_TABLES, ...MIGRATION_5_TABLES, ...MIGRATION_6_TABLES]) {
      counts[table] = (
...
describe("runner host construction: the migrated artifact is a real on-disk SQLite db", () => {
  it("produces an x-builder.db that re-opens with user_version 6 and the migration tables, with the JSON renamed", async () => {
