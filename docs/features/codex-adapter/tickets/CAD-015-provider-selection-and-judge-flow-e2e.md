...
- 2026-06-12 — **CONCERN RESOLVED** (user chose fix-now at triage): the two stale `/tmp/lj-judge-verdict.png` / `/tmp/lj-judge-unavailable.png` debug screenshot lines were removed from `judge-flow.spec.ts` (`5dba889`, test-only). judge-flow 2/2 + the full provider/judge e2e set 16/16 green.
